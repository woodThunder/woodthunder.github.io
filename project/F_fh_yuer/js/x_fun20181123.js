...
//	});
  	...
  	$(".more_video").click(function(){
  		$("html,body").stop().animate({scrollTop:$(".more_tit").offset().top});
  	})
...
//		console.log($W.index())
		...
		$L_p = (parseFloat($L.find(".p1 i").text())/100).toFixed(3);
//		console.log($L_p)
		$L.width(Math.floor(($W.width()-$M.width())*$L_p));
		
		$R_p = (parseFloat($R.find(".p1 i").text())/100).toFixed(3);
//		console.log($R_p)
		$R.width(Math.floor(($W.width()-$M.width())*$R_p));
		...
		if($L.width() < $L_txt.width()){
			if(Math.floor($L.find(".p1 i").text()) == 0){
				...
				$L.width(Math.floor($L_txt.width()+10));
		...
		if($R.width() < $R_txt.width()){
			if(Math.floor($R.find(".p1 i").text()) == 0){
				...
				$R.width(Math.floor($R_txt.width()+10));
