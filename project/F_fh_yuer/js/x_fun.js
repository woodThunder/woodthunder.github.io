...
$(document).ready(function () {
	...
	//按钮点击顶部视频播放
	$(".head .v_btn1").click(function () {
		$(".head .swiper-slide-active").find(".v2").show();
		$(".head .swiper-slide-active").find(".v2").get(0).play();
		//		$(".v1").get(0).currentTime = 0;
		$(".head .swiper-slide-active").find(".v2").get(0).addEventListener("pause",function(){
	    	$(this).hide();
		});
	...
	if (u.indexOf('Android') > -1 || u.indexOf('Linux') > -1){  // 安卓
		...
		$(".close").click(function () {
			$("body,html").scrollTop(scrollToTop);
			$('.main').removeClass('cur');
			// $(".wrap").show();
			$("#video").attr("src", "");
			$("body,html").removeClass('off');
	...
	$(".state").click(function () {
		scrollToTop = $(window).scrollTop();
		$(".layer,.layer_state").show();
		$("body,html").addClass("off");
	})
	close(".state_close", ".layer,.layer_state");
	
	//活动介绍切换
	$(".rule_tit").click(function(){
		if($(this).hasClass("on")){
			return false;
		}else{
			$(this).addClass("on").siblings().removeClass("on");
			$(".rule_list").eq($(this).index()).addClass("on").siblings().removeClass("on");
		}
	})
